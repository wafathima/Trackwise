...
import { Loader2, User, Mail, Lock, GraduationCap, Eye, EyeOff } from 'lucide-react';
...
const Register = () => {
  ...
  const { values, setValues, errors, handleChange, validate } = useForm({
    name: '', email: '', password: '', role: 'Student', classGroup: ''
  }, validationRules);

  const handleRoleChange = (e) => {
    const selectedRole = e.target.value;
    setValues({ ...values, role: selectedRole, classGroup: selectedRole === 'Student' ? '' : null });
  };
  ...
  return (
    ...
    >
      <style>{`
        ...
        .ledger-input:focus, .ledger-select:focus { border-color: ${MOSS} !important; box-shadow: 0 0 0 1px ${MOSS}55; }
        ...
        >
          <div className="text-center mb-6">
            <div
              ...
              style={{ backgroundColor: PAPER, borderColor: MOSS }}
            >
              <GraduationCap className="w-6 h-6" style={{ color: INK }} />
            </div>
            <h2 className="text-3xl" style={{ fontFamily: FONT_DISPLAY, fontWeight: 600 }}>Create Account</h2>
            <p className="mt-1 text-sm" style={{ color: `${INK}99` }}>Join Trackwise and level up your routine</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: `${INK}66` }} />
                <input
                  type="text"
                  name="name"
                  value={values.name}
              ...
              {errors.name && <p className="text-xs mt-1" style={{ color: REDINK }}>{errors.name}</p>}
            </div>
            ...
            <div>
              ...
              {errors.email && <p className="text-xs mt-1" style={{ color: REDINK }}>{errors.email}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="ledger-input w-full pl-11 pr-12 py-2.5 rounded-md border-2 outline-none transition"
                  style={fieldStyle}
                  placeholder="••••••••"
                />
                <button
              ...
              {errors.password && <p className="text-xs mt-1" style={{ color: REDINK }}>{errors.password}</p>}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="ledger-select w-full px-3 py-2.5 rounded-md border-2 outline-none transition"
                  style={fieldStyle}
                >
                  {['Student', 'Parent', 'Teacher', 'Mentor', 'Admin'].map(r => (
              ...
              {values.role === 'Student' && (
                <div>
                  ...
                  <div className="relative">
                    <GraduationCap className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: `${INK}66` }} />
                    <select
                      name="classGroup"
                      value={values.classGroup}
                      onChange={handleChange}
                      required
                      className="ledger-select w-full pl-9 pr-3 py-2.5 rounded-md border-2 outline-none transition"
                      style={fieldStyle}
                    >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-2 py-3 font-bold rounded-md shadow-md transition flex items-center justify-center gap-2 text-white active:scale-[0.98] disabled:opacity-50"
          ...
          <p className="text-center text-sm mt-6" style={{ color: `${INK}99` }}>
            Already have an account?{' '}
            <Link to="/login" className="font-semibold hover:underline" style={{ color: MOSS }}>
              Login here
            </Link>
          </p>
        </div>
