...
const MentorDashboard = ({ user }) => {
  ...
  const fetchData = async () => {
    ...
    setToast(null);
    try {
      // Fetch both mentees and stats in parallel
      const [menteesData, statsData] = await Promise.all([
        mentorService.getMentees(),
  ...
  const handleSendMessage = async (menteeId) => {
    setSubmitting(true);
    try {
      await mentorService.sendMessage(menteeId, messageForm);
      setToast({ message: 'Message sent successfully!', type: 'success' });
      setShowMessageModal(false);
      setMessageForm({ subject: '', content: '', type: 'motivation' });
      await fetchData(); // Refresh to get updated messages
    } catch (error) {
      console.error('Error sending message:', error);
      setToast({ message: 'Failed to send message', type: 'error' });
    } finally {
  ...
  const handleCreatePlan = async (menteeId) => {
    setSubmitting(true);
    try {
      await mentorService.createStudyPlan(menteeId, planForm);
      setToast({ message: 'Study plan created successfully!', type: 'success' });
      setShowPlanModal(false);
      setPlanForm({ title: '', description: '', subjects: '', schedule: '', resources: '' });
      await fetchData();
  ...
  const handleCreateWorkout = async (menteeId) => {
    setSubmitting(true);
    try {
      await mentorService.createWorkoutPlan(menteeId, workoutForm);
      setToast({ message: 'Workout plan created successfully!', type: 'success' });
      setShowWorkoutModal(false);
      setWorkoutForm({ title: '', type: 'fitness', exercises: '', duration: '', difficulty: 'beginner', notes: '' });
      await fetchData();
  ...
  const handleCreateCommunication = async (menteeId) => {
    setSubmitting(true);
    try {
      await mentorService.createCommunicationPlan(menteeId, communicationForm);
      setToast({ message: 'Communication plan created successfully!', type: 'success' });
      setShowCommunicationModal(false);
      setCommunicationForm({ title: '', type: 'speaking', description: '', activities: '', frequency: 'daily', notes: '' });
      await fetchData();
  ...
  const handleAwardBadge = async (menteeId) => {
    setSubmitting(true);
    try {
      await mentorService.awardBadge(menteeId, badgeForm);
      setToast({ message: `Badge "${badgeForm.name}" awarded successfully!`, type: 'success' });
      setShowBadgeModal(false);
      setBadgeForm({ name: '', description: '', category: 'achievement' });
      await fetchData();
  ...
  const avgDiscipline = mentees.length > 0 
    ? Math.round(mentees.reduce((acc, m) => acc + (m.disciplineScore || 0), 0) / mentees.length)
    : 0;
  const level = getDisciplineLevel(avgDiscipline);
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="p-4 rounded-sm" style={{ backgroundColor: CARD, border: `1px solid ${INK}14` }}>
                ...
                <div className="space-y-2">
                  {mentees.slice(0, 3).map((mentee, idx) => {
                    const discipline = getDisciplineLevel(mentee.disciplineScore || 0);
                    return (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-3 rounded-sm"
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                          <div className="flex items-center gap-3 text-xs">
                            <span className="flex items-center gap-1" style={{ color: MOSS }}>
                              <Sparkles className="w-3 h-3" />
                              {mentee.studyStreak || 0}d
                            </span>
                            <span className="flex items-center gap-1" style={{ color: SLATE }}>
                              <Dumbbell className="w-3 h-3" />
                              {mentee.workoutStreak || 0}d
                            </span>
                            <span className="flex items-center gap-1" style={{ color: BRASS }}>
                              <Award className="w-3 h-3" />
                              {mentee.badges?.length || 0}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto p-6" style={{
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => handleCreatePlan(selectedMentee.id)} disabled={submitting} className="flex-1 py-2 rounded-sm text-sm font-medium" style={{ backgroundColor: MOSS, color: CARD }}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto p-6" style={{
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => handleCreateWorkout(selectedMentee.id)} disabled={submitting} className="flex-1 py-2 rounded-sm text-sm font-medium" style={{ backgroundColor: SLATE, color: CARD }}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-sm max-w-lg w-full max-h-[90vh] overflow-y-auto p-6" style={{
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => handleCreateCommunication(selectedMentee.id)} disabled={submitting} className="flex-1 py-2 rounded-sm text-sm font-medium" style={{ backgroundColor: BRASS, color: CARD }}>
                {submitting ? 'Creating...' : 'Create Plan'}
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-sm max-w-lg w-full p-6" style={{
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => handleSendMessage(selectedMentee.id)} disabled={submitting} className="flex-1 py-2 rounded-sm text-sm font-medium flex items-center justify-center gap-2" style={{ backgroundColor: SLATE, color: CARD }}>
                {submitting ? <Loader className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} Send
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="rounded-sm max-w-lg w-full p-6" style={{
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => handleAwardBadge(selectedMentee.id)} disabled={submitting} className="flex-1 py-2 rounded-sm text-sm font-medium flex items-center justify-center gap-2" style={{ backgroundColor: BRASS, color: CARD }}>
                {submitting ? <Loader className="w-4 h-4 animate-spin" /> : <Award className="w-4 h-4" />} Award
              </button>
            </div>
          </div>
        </div>
