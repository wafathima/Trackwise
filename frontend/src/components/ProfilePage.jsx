...
const ProfilePage = () => {
  ...
  const [tempSettingValue, setTempSettingValue] = useState(null);
  ...
  const handleSettingToggle = (settingKey) => {
    ...
    setSuccess(`${settingKey.replace(/([A-Z])/g, ' $1').trim()} updated successfully!`);
  ...
  const handleThemeChange = (theme) => {
    ...
    setSuccess(`Theme changed to ${theme}!`);
    setTimeout(() => setSuccess(''), 3000);
  };
  ...
  const handleLanguageChange = (lang) => {
    ...
    setSuccess(`Language changed to ${lang === 'en' ? 'English' : 'Hindi'}!`);
    setTimeout(() => setSuccess(''), 3000);
  };
  ...
  const SettingsModal = ({ setting, value, onClose, onSave }) => {
    return (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="rounded-sm max-w-md w-full p-6 border" style={{ backgroundColor: CARD, borderColor: `${INK}22` }}>
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold" style={{ fontFamily: FONT_DISPLAY }}>
              {setting.replace(/([A-Z])/g, ' $1').trim()}
            </h3>
            <button onClick={onClose} className="transition-opacity hover:opacity-70" style={{ color: `${INK}60` }}>
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="space-y-4">
            <p className="text-sm" style={{ color: `${INK}70` }}>
              Current setting: <span className="font-semibold">{value ? 'Enabled' : 'Disabled'}</span>
            </p>
  ...
  // Theme Modal
  const ThemeModal = ({ currentTheme, onClose, onSave }) => {
          ...
          <div className="space-y-2">
            {themes.map((theme) => (
              <button
                ...
                className="w-full p-3 rounded-sm text-left flex items-center justify-between border transition-colors hover:bg-opacity-10"
                style={{
                  borderColor: currentTheme === theme ? MOSS : `${INK}20`,
                  backgroundColor: currentTheme === theme ? `${MOSS}10` : 'transparent'
                }}
              >
                <span className="text-sm capitalize">{theme}</span>
                {currentTheme === theme && <Check className="w-4 h-4" style={{ color: MOSS }} />}
              </button>
            ))}
          </div>
        </div>
  ...
  // Language Modal
  const LanguageModal = ({ currentLang, onClose, onSave }) => {
    const languages = [
          ...
          <div className="space-y-2">
            {languages.map((lang) => (
              <button
                ...
                className="w-full p-3 rounded-sm text-left flex items-center justify-between border transition-colors hover:bg-opacity-10"
                style={{
                  borderColor: currentLang === lang.code ? MOSS : `${INK}20`,
                  backgroundColor: currentLang === lang.code ? `${MOSS}10` : 'transparent'
                }}
              >
                <span className="text-sm">{lang.label}</span>
                {currentLang === lang.code && <Check className="w-4 h-4" style={{ color: MOSS }} />}
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen relative rounded-md" style={{ backgroundColor: PAPER, color: INK, fontFamily: FONT_BODY }}>
      ...
      <div className="pl-11 pr-5 md:pr-8 py-6 space-y-6">
        ...
        {/* Settings Section - Now with Working Functionality */}
        <Panel>
          <SectionHeading icon={<Settings className="w-5 h-5" />} title="Settings" accent={SLATE} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            {/* Notification Settings */}
            <div 
              className="flex items-center justify-between p-3 rounded-sm border transition-shadow hover:shadow-sm cursor-pointer"
              style={{ borderColor: `${INK}0F` }}
              onClick={() => handleSettingToggle('notifications')}
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: `${BRASS}12`, color: BRASS }}>
                  <Bell className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-sm font-medium">Notifications</p>
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>Manage notification preferences</p>
                </div>
              </div>
              <div 
                className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer setting-slider`}
                style={{ 
                  backgroundColor: settings.notifications ? MOSS : `${INK}30`,
                }}
              >
                <div 
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-300`}
                  style={{ 
                    transform: settings.notifications ? 'translateX(22px)' : 'translateX(2px)',
                  }}
                />
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: `${SLATE}12`, color: SLATE }}>
                  <Lock className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-sm font-medium">Privacy</p>
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>Control your privacy settings</p>
                </div>
              </div>
              <div 
                className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer setting-slider`}
                style={{ 
                  backgroundColor: settings.privacy ? MOSS : `${INK}30`,
                }}
              >
                <div 
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-300`}
                  style={{ 
                    transform: settings.privacy ? 'translateX(22px)' : 'translateX(2px)',
                  }}
                />
              </div>
            </div>
            ...
            <div 
              className="flex items-center justify-between p-3 rounded-sm border transition-shadow hover:shadow-sm cursor-pointer"
              style={{ borderColor: `${INK}0F` }}
              onClick={() => setShowSettingsModal('theme')}
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: `${INK}12`, color: INK }}>
                  <Moon className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-sm font-medium">Theme</p>
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>
                    {settings.theme === 'light' ? 'Light' : settings.theme === 'dark' ? 'Dark' : 'System'} mode
                  </p>
                </div>
              </div>
            ...
            <div 
              className="flex items-center justify-between p-3 rounded-sm border transition-shadow hover:shadow-sm cursor-pointer"
              style={{ borderColor: `${INK}0F` }}
              onClick={() => setShowSettingsModal('language')}
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: `${MOSS}12`, color: MOSS }}>
                  <Globe className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-sm font-medium">Language</p>
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>
                    {settings.language === 'en' ? 'English' : settings.language === 'hi' ? 'Hindi' : 'Malayalam'}
                  </p>
                </div>
              </div>
              <ChevronRight className="w-4 h-4" style={{ color: `${INK}45` }} />
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>Two-factor authentication</p>
                </div>
              </div>
              <div 
                className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer setting-slider`}
                style={{ 
                  backgroundColor: settings.twoFactorAuth ? MOSS : `${INK}30`,
                }}
              >
                <div 
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-300`}
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: `${REDINK}12`, color: REDINK }}>
                  <Volume2 className="w-4 h-4" />
                </div>
                <div>
                  <p className="text-sm font-medium">Sound Effects</p>
                  <p className="text-[10px]" style={{ color: `${INK}60` }}>Toggle sound effects</p>
                </div>
              </div>
              <div 
                className={`w-10 h-5 rounded-full relative transition-colors cursor-pointer setting-slider`}
                style={{ 
                  backgroundColor: settings.soundEffects ? MOSS : `${INK}30`,
                }}
              >
                <div 
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-300`}
                  style={{ 
                    transform: settings.soundEffects ? 'translateX(22px)' : 'translateX(2px)',
                  }}
                />
              </div>
            </div>
